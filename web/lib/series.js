...
export function showsCompetitiveStats(member) {
  ...
  const name = String(member.gamertag || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  const display = String(member.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  if (name === 'naevii' || name === 'naeviiszn' || name.startsWith('naeviiszn') || display === 'naevii' || display === 'naeviiszn') return true;
  const title = String(member.title || '').toLowerCase();
  if (/\borg\s*owner\b/.test(title) || title === 'owner') return true;
  if (member.slot === 'staff' || member.slot === 'fa') return false;
  return member.slot === 'starter' || member.slot === 'bench' || !member.slot;
}
