import { aggregate, statsByKey, statsForMember, teamAvgDamage, teamKD, teamWinRate } from './stats';
...
function modeRows(matches) {
  return statsByKey(matches, (m) => m.mode || 'Unknown').map((s) => [s.key, String(s.total), `${s.wins}-${s.losses}`, `${s.winRate}%`]);
}

function mapRows(matches) {
  return statsByKey(matches, (m) => m.map || 'Unknown').map((s) => [s.key, String(s.total), `${s.wins}-${s.losses}`, `${s.winRate}%`]);
}
  ...
  return members
    .map((member) => {
      ...
      return {
        ...
        agg,
        row: [member.gamertag, String(agg.matches), String(agg.kd), String(agg.kills), String(agg.deaths), String(agg.matches ? Math.round(agg.damage / agg.matches) : 0), `${agg.winRate}%`],
      };
    })
...
export function toMarkdown(report) {
  const lines = [`# ${report.title}`, '', `_${report.subtitle}_`, '', `Generated ${String(report.generatedAt).slice(0, 10)}`, ''];
...
export function buildTeamReport({ team, matches = [], members = [], scrims = [] }) {
  ...
  const sections = [
    ...
    { heading: 'Players', columns: ['Player', 'Matches', 'K/D', 'Kills', 'Deaths', 'Avg DMG', 'Win %'], rows: playerRows(matches, members) },
    { heading: 'Recent Matches', columns: ['Date', 'Opponent', 'Mode', 'Map', 'Score', 'Result'], rows: recentRows(matches) },
  ];
  if (scrims.length) {
...
export function buildOpponentReport({ opponent, matches = [] }) {
  ...
  if ((opponent?.players || []).length) {
    sections.push({ heading: 'Roster', columns: ['Player', 'Role', 'Note'], rows: opponent.players.map((p) => [p.gamertag, p.role, p.note || '—']) });
  }
  if ((opponent?.map_notes || []).length) {
    sections.push({ heading: 'Map & Mode Notes', columns: ['Map', 'Mode', 'Threat', 'Note'], rows: opponent.map_notes.map((m) => [m.map, m.mode, m.threat, m.note || '—']) });
  }
  if (h2h.length) {
    sections.push({ heading: 'Head-to-Head Matches', columns: ['Date', 'Mode', 'Map', 'Score', 'Result'], rows: recentRows(h2h).map((r) => [r[0], r[2], r[3], r[4], r[5]]) });
  }
